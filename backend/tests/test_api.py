...
def test_boss_extension_can_be_downloaded(client, admin_headers):
    ...
    with zipfile.ZipFile(BytesIO(response.data)) as archive:
        ...
        assert manifest["version"] == "0.3.25"
        ...
        assert "collectNewGreetingOnlineResumeList" in content
        assert "waitForResumeTextReady" in content
        assert "isObtainedResumeListPage" in content
        assert "can_import_obtained_resume" in content
        assert "resolveBossAttachmentUrl" in content
        ...
        assert "XMLHttpRequest" in probe
        assert "window.fetch" in probe
