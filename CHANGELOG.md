...
- 修复 AI 深度洞察页面的中文编码损坏和 TypeScript 类型缺失，综合报告改为单次调用 `/api/insight/report`。
- 深度洞察改用轻量 CSS 图形并移除 Recharts 运行依赖，恢复前端 TypeScript + Vite 生产构建。
- 修正招聘漏斗统计口径：按候选人-岗位流程的最高到达阶段形成单调漏斗，淘汰独立统计，并修正跨岗位招聘周期。
- 洞察手动刷新支持绕过服务端短时缓存，无样本指标统一显示为暂无数据。
- 恢复候选人详情路由的注册和角色校验，修复详情审计、负责人隔离、BOSS 简历合并、异步重解析和标签治理的连锁回归。
- 修复组织后代节点查询函数的作用域错误，恢复部门员工列表和组织概览接口。
- 将六个 AI 深度洞察接口从总路由文件拆分到独立 Blueprint，并统一无数据时的响应字段和类型。
- 生产巡检脚本移除固定生产地址、root 默认账号和明文密码，改用 SSH Agent/密钥与 `known_hosts` 主机身份校验。
- 密钥扫描扩展到未跟踪但未忽略的工作区文件，并增加长明文密码赋值检测。
- API 文档补充六个 `/api/insight/*` 深度洞察接口。
- 部署和运维文档统一以 Docker Compose 为推荐生产方式，并移除真实生产 IP。
- 明确仓库已经包含 Playwright E2E 冒烟测试和 GitHub Actions 流水线；仍需扩展 E2E 业务覆盖率。
...
- Playwright 当前只有登录、简历上传入口、岗位页和 Agent 输入的冒烟覆盖，仍需补充完整招聘闭环和失败路径测试。
