...
- 多文件/ZIP 简历上传
- BOSS 岗位/候选人/单页简历导入日志、明细追踪和失败重试接口
- 通知中心已接入候选人入库、岗位匹配、流程推进、面试、Offer、BOSS 同步等业务事件
- AI 面试语音接口：面试间 ASR/TTS 状态、浏览器语音识别回传、播报任务、语音调用日志
...
- 面试管理：面试安排、AI 面试题、候选人网页面试间、ASR/TTS 语音留痕、动态追问、AI 评分、报告导出
- Offer 管理：Offer 创建、状态流转、确认函导出、流程同步
- BOSS 闭环：插件下载、登录态绑定、岗位同步、候选人导入、同步日志、失败重试、BOSS 候选人推荐
...
- 运维能力：健康检查、结构化访问日志、审计日志、通知中心、后台任务、数据清理、备份/恢复脚本
...
- `GET /api/public/interview-room/<token>/speech/status`
- `POST /api/public/interview-room/<token>/speech/asr`
- `POST /api/public/interview-room/<token>/speech/tts`
...
- `GET /api/boss/sync/jobs`
- `GET /api/boss/sync/jobs/<id>`
- `POST /api/boss/sync/jobs/<id>/retry`
...
- BOSS 插件真实页面采集稳定性和更多页面结构适配
- AI 面试外部 ASR/TTS Provider 适配、设备检测、异常恢复
- Redis/Celery 或更稳定的后台任务队列
- 对象存储或专用文件服务
- 前端 Playwright/E2E 自动化测试
- 集中日志、错误告警与外部通知渠道真实发送适配
