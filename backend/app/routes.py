...
@api.get("/ops/deploy-gates")
@login_required
@roles_required("admin", "manager")
def ops_deploy_gates(user):
    report = build_deploy_gate_report()
    audit_log(user, "view", "ops_deploy_gates", None, "deploy_gates")
    db.session.commit()
    return ok(report)
...
def build_agent_execution_plan(user, message, pending_action=None, history=None):
    ...
    heuristic_tools = infer_agent_tool_names(text, pending_action)
    heuristic_tool = heuristic_tools[0]
    base_plan = [
    ...
    planner = {
        ...
        "plan": base_plan,
        "selected_tools": [{"name": name, "reason": "本地规划器根据问题关键词、实体和上下文选择。"} for name in heuristic_tools],
...
def infer_agent_tool_name(text, pending_action=None):
    return infer_agent_tool_names(text, pending_action)[0]


def infer_agent_tool_names(text, pending_action=None):
    if pending_action:
    ...
    if "面试" in value:
        tools.append("get_interview_schedule")
    if "漏斗" in value or "流程" in value or "阶段" in value:
        tools.append("get_pipeline_funnel")
    ...
    if "boss" in lowered:
        tools.append("get_boss_status")
    if "用户" in value or "账号" in value or "权限" in value:
        tools.append("get_user_summary")
    if is_bi_request(value):
        tools.append("get_bi_snapshot")
    if not tools:
        tools.append("chat")
...
def build_agent_tool_calls(selected_tools, result):
    ...
    if executed == "agent_toolchain" and isinstance(payload, dict):
        calls = []
        for item in payload.get("chain") or []:
            tool_name = item.get("tool")
            if tool_name:
                calls.append({"name": tool_name, "status": "succeeded", "readonly": True, "summary": summarize_agent_tool_result(item)})
        return calls[:6]
    calls = []
...
def should_run_agent_multi_tool_chain(planner, pending_action=None, text=""):
    if pending_action:
        return False
    selected = [item.get("name") for item in (planner or {}).get("selected_tools") or [] if item.get("name")]
    selected = [name for name in dict.fromkeys(selected) if name in agent_readonly_tool_names()]
    if len(selected) < 2:
        return False
    if "create_job" in selected or "agent_plan" in selected:
        return False
    if not is_explicit_multi_step_agent_request(text):
        return False
    return True


def is_explicit_multi_step_agent_request(text):
    value = str(text or "")
    return any(keyword in value for keyword in ["同时", "顺便", "然后", "再查", "再看", "再查看", "一起", "综合", "并查看", "并查询", "并统计"])


def agent_readonly_tool_names():
    return {
        "global_lookup",
        "knowledge_lookup",
        "get_candidate_segment_stats",
        "get_candidate_experience_stats",
        "search_candidates",
        "get_job_summary",
        "match_candidates_for_job",
        "analyze_employee_resume",
        "get_pipeline_funnel",
...
def run_agent_multi_tool_chain(user, message, planner, history=None):
    text = str(message or "").strip()
    suggestions = [
        "继续深挖第一位人选",
        "按当前结论生成下一步动作",
    ...
    selected = [item.get("name") for item in (planner or {}).get("selected_tools") or [] if item.get("name") in agent_readonly_tool_names()]
    selected = list(dict.fromkeys(name for name in selected if name != "chat"))[:4]
    if not selected:
        selected = [infer_agent_tool_name(text)]
    tool_results = []
    for tool_name in selected:
        result = execute_agent_readonly_tool(user, text, tool_name, history=history)
        if result:
            tool_results.append(result)
    if not tool_results:
        return run_agent_tool(user, message, None, history=history)
    answer = synthesize_agent_multi_tool_answer(text, tool_results, planner, history=history)
    return {
        "answer": answer,
        "tool": "agent_toolchain",
        "result": {
            "chain": [{"tool": item.get("tool"), "answer": item.get("answer"), "result": item.get("result")} for item in tool_results],
            "plan": [step.get("step") for step in (planner or {}).get("plan") or []],
            "mode": (planner or {}).get("mode", "rules"),
        },
...
def execute_agent_readonly_tool(user, text, tool_name, history=None):
    suggestions = []
    if tool_name == "global_lookup":
        return answer_global_knowledge_lookup(user, text, suggestions, history=history)
    if tool_name == "knowledge_lookup":
        return answer_profile_knowledge_lookup(user, text, suggestions, history=history)
    if tool_name == "analyze_employee_resume":
        if is_employee_resume_agent_request(text) or find_candidate_for_agent_message(text)[0] or find_employee_for_agent_message(text)[0]:
            return analyze_employee_resume_from_agent(user, text, suggestions, history=history)
        return None
    if tool_name == "get_candidate_segment_stats":
    ...
    if tool_name == "get_candidate_experience_stats":
        ...
        return {"answer": f"当前人才库经验档位共统计 {sum(item['count'] for item in data)} 人。", "tool": "get_candidate_experience_stats", "result": data, "suggestions": suggestions, "readonly": True}
    ...
    if tool_name == "match_candidates_for_job":
        ...
        if results:
            db.session.commit()
        first = results[0] if results else {"job": None, "items": []}
        return {"answer": format_match_results(results) if results else "当前还没有岗位，无法执行候选人推荐。", "tool": "match_candidates_for_job", "result": {"job": first["job"], "items": first["items"], "jobs": results}, "suggestions": suggestions, "readonly": True}
    if tool_name == "search_candidates":
        items = search_candidates_for_agent(text)
        return {"answer": f"找到 {len(items)} 位相关候选人，已按关键词和标签命中排序。", "tool": "search_candidates", "result": {"items": items}, "suggestions": suggestions, "readonly": True}
    if tool_name == "get_pipeline_funnel":
        ...
        return {"answer": f"当前流程中活跃候选人 {active_count} 人，Offer 阶段 {result.get('offer', 0)} 人，已入职 {result.get('onboarded', 0)} 人。", "tool": "get_pipeline_funnel", "result": result, "suggestions": suggestions, "readonly": True}
    if tool_name == "get_interview_schedule":
        ...
        return {"answer": f"当前共有 {len(assignments)} 条面试安排，待反馈 {counts.get('scheduled', 0)} 条，已反馈 {counts.get('completed', 0)} 条。", "tool": "get_interview_schedule", "result": {"counts": dict(counts), "upcoming": [item.to_dict() for item in assignments[:8]]}, "suggestions": suggestions, "readonly": True}
    if tool_name == "get_offer_status":
        offers = OfferRecord.query.order_by(OfferRecord.updated_at.desc()).all()
        counts = Counter(offer.status for offer in offers)
        return {"answer": f"当前共有 {len(offers)} 条 Offer，已接受 {counts.get('accepted', 0)} 条，已发放待确认 {counts.get('sent', 0)} 条。", "tool": "get_offer_status", "result": {"counts": {status: counts.get(status, 0) for status in sorted(OFFER_STATUSES)}, "recent": [item.to_dict() for item in offers[:5]]}, "suggestions": suggestions, "readonly": True}
    if tool_name == "get_boss_status":
        ...
        account = latest_boss_account(user)
        return {"answer": f"BOSS 当前为半自动模式，已同步候选人 {len(inbox)} 位，BOSS 岗位 {len(jobs)} 个。", "tool": "get_boss_status", "result": {"status": {"cookie_bound": bool(account), "account": account.account if account else "未绑定", "mode": "半自动", "can_auto_send": False}, "inbox": inbox, "jobs": [job.to_dict() for job in jobs]}, "suggestions": suggestions, "readonly": True}
    if tool_name == "get_user_summary" and user.role == "admin":
        users = User.query.order_by(User.id.asc()).all()
        roles = Counter(item.role for item in users)
        return {"answer": f"当前共有 {len(users)} 个账号，管理员 {roles.get('admin', 0)} 个，招聘经理 {roles.get('manager', 0)} 个，HR {roles.get('recruiter', 0)} 个，面试官 {roles.get('interviewer', 0)} 个。", "tool": "get_user_summary", "result": {"roles": dict(roles), "items": [with_permissions(item) for item in users]}, "suggestions": suggestions, "readonly": True}
    if tool_name == "get_bi_snapshot":
        snapshot = bi_snapshot()
        return {"answer": f"当前人才库 {snapshot['total_candidates']} 人，开放岗位 {snapshot['active_jobs']} 个，Offer 已接受 {snapshot['offer_status'].get('accepted', 0)} 条。", "tool": "get_bi_snapshot", "result": snapshot, "suggestions": suggestions, "readonly": True}
    return None


def synthesize_agent_multi_tool_answer(text, tool_results, planner, history=None):
    if llm_available():
        try:
            payload = [
                {
                    ...
                    "summary": summarize_agent_tool_result(item),
                    "result": compact_agent_tool_payload(item.get("result")),
                }
                ...
                [
                    {
                        "role": "system",
                        "content": (
                            "你是 HireInsight 招聘 Agent。根据多个工具的真实结果综合回答。"
                            "必须说明你先后调用了哪些工具、关键发现、冲突/不确定点和下一步建议。"
                            "不能编造工具结果之外的数据。输出 JSON：{\"answer\":\"中文回答\"}"
                        ),
                    },
                    {
                        "role": "user",
                        "content": json.dumps(
                            {
                                "question": text,
                                "planner": {"mode": (planner or {}).get("mode"), "intent": (planner or {}).get("intent"), "plan": (planner or {}).get("plan")},
                                "history": format_agent_history(history or []),
            ...
            pass
    lines = ["我按多步工具链完成了这次查询："]
    for index, item in enumerate(tool_results, 1):
        lines.append(f"{index}. {item.get('tool')}：{summarize_agent_tool_result(item)}")
        if item.get("answer"):
            lines.append(summarize_text(item.get("answer"), 220))
    lines.append("以上为系统内真实数据汇总；需要写入或推进流程时，我会等你确认后再执行。")
    return "\n".join(lines)


def compact_agent_tool_payload(payload):
...
def analyze_employee_resume_from_agent(user, text, suggestions, history=None):
    ...
    if not employee:
        candidate, candidate_alternatives = find_candidate_for_agent_message(text)
        if candidate:
            return analyze_candidate_resume_from_agent(user, candidate, text, suggestions, history=history)
        candidate_lines = "\n".join(
        ...
        talent_lines = "\n".join(
            f"- {item.name_masked}：{item.title or '职位未维护'}，{item.city or '城市未知'}，编号 {item.id}"
            for item in candidate_alternatives[:6]
        )
        answer = "我需要先确认要分析哪位员工或候选人。请带上姓名、员工编号、手机号后四位，或在问题里写清楚当前职位。"
        if candidate_lines:
            answer += f"\n可能相关的员工：\n{candidate_lines}"
        if talent_lines:
            answer += f"\n可能相关的人才库候选人：\n{talent_lines}"
        return {
            ...
            "result": {
                "matched": False,
                "employees": [employee_payload(item, user) for item in alternatives[:6]],
                "candidates": [item.to_dict() for item in candidate_alternatives[:6]],
            },
            "suggestions": ["查看张三的简历并推荐适合岗位", "分析员工 张三 的简历，推荐适合岗位", "给这个候选人的简历优化建议", "推荐内部调岗岗位"],
            "readonly": True,
        }
    ...
    transfer_items = [item for item in ranked if not item.get("current")][:5]
    replacement_items = employee_replacement_candidates_for_agent(employee, limit=5)
    result = {
        "matched": True,
        "plan": employee_agent_plan(employee),
        "employee": employee_payload(employee, user, detail=True),
        "items": [employee_job_match_payload(item) for item in top_items],
        "transfer_recommendations": [employee_job_match_payload(item) for item in transfer_items],
        "replacement_candidates": replacement_items,
        "current_job_analysis": current_analysis,
        "resume_optimizations": fallback_resume_optimizations(employee, top_items),
        "communication_advice": fallback_employee_communication_advice(employee, transfer_items, current_analysis),
        "llm": "disabled",
    }
    ...
    if llm_available():
        try:
            llm_payload = deepseek_employee_resume_review(user, employee, top_items, current_analysis, replacement_items, history=history)
            result["llm"] = "deepseek"
            ...
            if llm_payload.get("communication_advice"):
                result["communication_advice"] = llm_payload["communication_advice"]
        except LLMError as exc:
            result["llm"] = "error"
...
def find_candidate_for_agent_message(text):
    ...
    for candidate in candidates:
        ...
        compact_fields = [re.sub(r"\s+", "", field).lower() for field in fields if field]
        if any(field and field in normalized for field in compact_fields[:3]):
            exact.append(candidate)
            continue
        score = sum(2 for field in compact_fields if field and field in normalized)
...
def analyze_candidate_resume_from_agent(user, candidate, text, suggestions, history=None):
    jobs = apply_job_scope(Job.query.filter_by(status="active"), "recruiting").order_by(Job.created_at.desc()).all()
    if not jobs:
        jobs = Job.query.filter_by(status="active").order_by(Job.created_at.desc()).all()
    if not jobs:
        return {
            "answer": f"已找到候选人 {candidate.name_masked}，但当前没有开放招聘岗位可用于匹配。请先维护岗位 JD。",
            "tool": "analyze_employee_resume",
            "result": {"matched": True, "profile_type": "candidate", "candidate": candidate.to_dict(detail=True), "items": []},
            "suggestions": ["新建招聘岗位 JD", "导入岗位 JD", "查看人才库", "继续优化这份简历"],
            "readonly": True,
        }
    inferred_tags = candidate_tags_for_agent(candidate, jobs)
    ranked = []
    for job in jobs:
        structured = ensure_jd_structured(job)
        ...
        if reason["score"] >= 80:
            ...
            reason["summary"] = "候选人与该岗位存在一定匹配，建议结合项目细节进一步确认。"
        else:
            reason["summary"] = "候选人与该岗位关键要求差距较明显，不建议优先推荐。"
        ranked.append({"job": job, "score": reason["score"], "reason": reason})
    ranked.sort(key=lambda item: item["score"], reverse=True)
    top_items = ranked[:5]
    result = {
        "matched": True,
        "profile_type": "candidate",
        "plan": candidate_agent_plan(),
        "candidate": candidate.to_dict(detail=True),
        "items": [candidate_job_match_payload(item) for item in top_items],
        "resume_optimizations": fallback_candidate_resume_optimizations(candidate, top_items),
        "llm": "disabled",
    }
    llm_payload = None
    if llm_available():
        try:
            llm_payload = deepseek_candidate_resume_review(candidate, top_items, history=history)
            result["llm"] = "deepseek"
            result["ai_review"] = llm_payload
            if llm_payload.get("recommended_jobs"):
                result["ai_recommended_jobs"] = llm_payload["recommended_jobs"]
            ...
            result["llm_error"] = str(exc)[:200]
    return {
        "answer": format_candidate_agent_answer(candidate, top_items, result, llm_payload),
        "tool": "analyze_employee_resume",
        "result": result,
        "suggestions": [
            f"把 {candidate.name_masked} 加入最佳岗位流程",
            f"继续优化 {candidate.name_masked} 的简历",
            "给这个候选人生成沟通话术",
            "换一个岗位重新匹配",
        ],
        "readonly": True,
...
def candidate_tags_for_agent(candidate, jobs):
    tags = {tag.tag: int(tag.score or 3) for tag in candidate.tags}
    context = " ".join([candidate.name_masked or "", candidate.title or "", (candidate.resume_json or {}).get("summary") or "", candidate.raw_text or ""])
    context_lower = context.lower()
    for job in jobs:
        structured = ensure_jd_structured(job)
        for skill in parse_skill_tags(structured.get("skill_tags_raw")):
            tag = str(skill.get("tag") or "").strip()
            if tag and tag.lower() in context_lower and tag not in tags:
                tags[tag] = 4
    return [{"tag": tag, "score": max(1, min(5, score)), "category": ""} for tag, score in tags.items()]


def candidate_agent_plan():
    return [
        "识别人才库候选人并读取完整简历",
        "读取开放招聘岗位 JD",
        "先用规则算法计算候选人与岗位的匹配分",
        "调用 DeepSeek 阅读完整简历和 JD，校准岗位推荐与理由",
        "输出简历优化建议和下一步招聘动作",
    ]


def candidate_job_match_payload(item):
    return {"job": item["job"].to_dict(), "score": item["score"], "reason": item["reason"]}


def fallback_candidate_resume_optimizations(candidate, top_items):
    missing = []
    for item in top_items[:3]:
        missing.extend(item["reason"].get("missing_tags", [])[:3])
    missing = list(dict.fromkeys(missing))[:6]
    suggestions = [
        "简历开头增加 3-5 行个人简介，明确目标岗位、年限、核心技术/业务能力。",
        "项目经历按“项目背景-个人职责-技术/业务动作-量化结果”展开，避免只有职责描述。",
        "把与目标岗位相关的技能放到每段项目经历中提供证据，而不是只堆技能词。",
    ]
    if missing:
        suggestions.append("若候选人实际具备这些能力，建议在简历中补充证据：" + "、".join(missing) + "。")
    if not (candidate.raw_text or "").strip():
        suggestions.append("当前缺少完整简历原文，建议上传原始附件后再做深度分析。")
    return suggestions


def deepseek_candidate_resume_review(candidate, top_items, history=None):
    ...
    data = chat_json(
        [
            {
                "role": "system",
                "content": (
                    "你是招聘 Agent。必须阅读候选人完整简历、开放岗位 JD 和规则匹配结果，判断候选人适合哪些岗位，"
                    "给出匹配理由、风险、简历优化建议和下一步招聘动作。不要只复述标签。"
                    "输出 JSON：{\"summary\":\"\",\"recommended_jobs\":[{\"job_id\":1,\"title\":\"\",\"score\":0,\"reason\":\"\",\"gaps\":[\"\"]}],"
                    "\"resume_optimizations\":[\"\"],\"risks\":[\"\"],\"next_actions\":[\"\"]}。score 为 0-100。"
                ),
            },
            {
                "role": "user",
                "content": (
                    ...
                    f"候选人={candidate.name_masked} 当前标题={candidate.title or ''} 城市={candidate.city or ''}\n"
                    f"简历摘要={resume.get('summary') or ''}\n"
                    f"完整简历={summarize_text(candidate.raw_text or resume.get('summary') or '', 2800)}\n"
                    f"候选岗位={jobs_payload}"
                ),
            },
        ],
        ...
        timeout=35,
        source="agent",
        tool_name="analyze_candidate_resume",
    )
    ...
    data["next_actions"] = [str(item).strip() for item in data.get("next_actions") or [] if str(item).strip()][:6]
    return data


def format_candidate_agent_answer(candidate, top_items, result, llm_payload=None):
    if llm_payload:
        summary = str(llm_payload.get("summary") or "").strip()
        jobs = llm_payload.get("recommended_jobs") or []
    else:
        summary = "已基于候选人简历和开放岗位 JD 做规则匹配；DeepSeek 未启用或暂不可用时，会先给出规则版建议。"
        jobs = [
            {
                "title": item["job"].title,
                "score": item["score"],
    ...
    lines = [f"已分析候选人 {candidate.name_masked}（{candidate.title or '职位未维护'}）。"]
    plan = result.get("plan") or []
    if plan:
        lines.append("本次执行：" + " -> ".join(plan))
    if summary:
    ...
    if optimizations:
        ...
        for item in optimizations[:5]:
            lines.append(f"- {item}")
    if llm_payload and llm_payload.get("risks"):
        lines.append("")
        lines.append("风险点：")
        for item in llm_payload["risks"][:4]:
            lines.append(f"- {item}")
    if llm_payload and llm_payload.get("next_actions"):
        lines.append("")
        lines.append("下一步动作：")
        for item in llm_payload["next_actions"][:4]:
            lines.append(f"- {item}")
    if result.get("llm") == "deepseek":
        lines.append("")
        lines.append("本次结论已调用 DeepSeek 阅读候选人完整简历和岗位 JD。")
...
def employee_agent_plan(employee):
    steps = [
        "识别员工身份并读取内部员工档案",
        "读取员工简历原文、解析摘要、技能标签和当前岗位",
        "筛选公司内部开放岗位并结构化岗位 JD",
        "先用规则算法计算岗位匹配分、命中能力和缺失能力",
        "调用 DeepSeek 阅读完整上下文，校准岗位推荐、风险和简历优化建议",
    ]
    if employee.current_job:
        steps.append("基于员工当前岗位 JD 推荐离职替补候选人")
    return steps


def employee_replacement_candidates_for_agent(employee, limit=5):
    if not employee.current_job:
        return []
    existing_employee_candidate_ids = {
        item.candidate_id
        for item in EmployeeProfile.query.filter(EmployeeProfile.candidate_id.isnot(None)).all()
        if item.candidate_id
    }
    items = []
    for item in preview_matches(employee.current_job, limit=30):
        candidate_id = item.get("candidate_id")
        if not candidate_id or candidate_id == employee.candidate_id or candidate_id in existing_employee_candidate_ids:
            continue
        candidate = db.session.get(Candidate, candidate_id)
        if not candidate:
            continue
...
def fallback_employee_communication_advice(employee, transfer_items, current_analysis):
    best = transfer_items[0] if transfer_items else None
    lines = [
        f"先和 {employee.name} 确认职业发展意愿、近期项目贡献和对当前岗位的满意度。",
        "沟通时不要直接下结论，先说明本次分析基于简历、岗位 JD 和系统数据，需要结合绩效与主管反馈复核。",
    ]
    if best:
        lines.append(f"如果讨论调岗，可优先围绕「{best['job'].title}」展开，说明匹配优势和需要补齐的能力。")
    if current_analysis:
        salary = (current_analysis.get("analysis") or {}).get("salary") or {}
        if salary.get("status") == "low":
            lines.append("当前存在薪资偏低信号，若员工匹配度较高，建议同步评估保留风险。")
        elif salary.get("status") == "high":
            lines.append("当前存在薪资偏高信号，建议结合岗位产出和绩效复核成本合理性。")
    return lines
...
def deepseek_employee_resume_review(user, employee, top_items, current_analysis, replacement_items=None, history=None):
        ...
        [
            {
                ...
                "content": (
                    ...
                    "输出 JSON：{\"summary\":\"\",\"recommended_jobs\":[{\"job_id\":1,\"title\":\"\",\"score\":0,\"reason\":\"\",\"gaps\":[\"\"]}],"
                    "\"resume_optimizations\":[\"\"],\"risks\":[\"\"],\"communication_advice\":[\"\"],\"replacement_view\":\"\","
                    "\"memory_notes\":[\"\"]}。score 为 0-100。"
                ),
            },
            {
                "role": "user",
                "content": (
                    ...
                    f"候选岗位={jobs_payload}\n"
                    f"离职替补候选人={replacement_items or []}"
                ),
            },
        ],
        temperature=0.25,
    ...
    data["resume_optimizations"] = [str(item).strip() for item in data.get("resume_optimizations") or [] if str(item).strip()][:8]
    data["risks"] = [str(item).strip() for item in data.get("risks") or [] if str(item).strip()][:6]
    data["communication_advice"] = [str(item).strip() for item in data.get("communication_advice") or [] if str(item).strip()][:6]
    data["replacement_view"] = str(data.get("replacement_view") or "").strip()[:800]
...
def format_employee_agent_answer(employee, top_items, result, llm_payload=None):
    ...
    plan = result.get("plan") or []
    if plan:
        lines.append("本次执行：" + " -> ".join(plan[:5]))
    if summary:
        lines.append(summary)
    current_analysis = result.get("current_job_analysis")
    if current_analysis:
        analysis = current_analysis.get("analysis") or {}
        lines.append(
            f"当前岗位/薪资：岗位匹配 {current_analysis.get('match_score')}/100，"
            f"薪资评分 {current_analysis.get('salary_score')}/100，"
            f"风险 {risk_label(current_analysis.get('risk_level'))}。{analysis.get('summary') or ''}"
        )
    lines.append("")
    lines.append("推荐岗位：")
    for index, item in enumerate(jobs[:5], start=1):
    ...
    if optimizations:
        ...
        for item in optimizations[:5]:
            lines.append(f"- {item}")
    replacements = result.get("replacement_candidates") or []
    if replacements:
        lines.append("")
        lines.append("离职替补候选人：")
        for item in replacements[:3]:
            candidate = item.get("candidate") or {}
            reason = item.get("reason") or {}
            hits = "、".join(hit.get("candidate_tag", "") for hit in reason.get("hits", [])[:3] if hit.get("candidate_tag"))
            lines.append(f"- {candidate.get('name_masked')}：{item.get('score')}/100，{candidate.get('title') or '职位未维护'}" + (f"，命中 {hits}" if hits else ""))
    if llm_payload and llm_payload.get("replacement_view"):
        lines.append(f"替补判断：{llm_payload['replacement_view']}")
    communication = result.get("communication_advice") or []
    if communication:
        lines.append("")
        lines.append("沟通建议：")
        for item in communication[:4]:
            lines.append(f"- {item}")
    if result.get("llm") == "deepseek":
        lines.append("")
...
def json_text_path(column, *path):
    """Return a text JSON-path expression that compiles for every supported DB."""
    expression = column
    for key in path:
